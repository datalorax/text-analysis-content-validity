...
abstract: |
  ...
  which various summary statistics are then calculated [e.g.,
  categorical concurrence, balance of representation; @webb99]. In this
  paper, we propose an approach that builds upon traditional methods 
  for alignment by leveraging text-based machine learning procedures,
  specifically topic modeling, to identify text-based clusters within the content
  standards. The probability that each item from a statewide assessment aligns 
  with each cluster/topic can then be estimated as an additional source of
  content-related validity evidence. We discuss the utility of this approach, 
  and show how visualizations can be used to evaluate to overall coverage of the
  content standards by the test items.
