...
Some studies have examined how newer, computational methods can add content-related validity evidence. 
For example, Sherin (2013) used a simple Natural Language Processing technique in order to identify common groups of responses to transcribed interview answers to questions about a scientific phenomenon. This work demonstrated how a newer, computational tool can be used to establish the construct validity that can be gained by using these very different ways of carrying out research about complex phenomena and processes. In another study, Beggrow et al. (2014) employed an approach similar to Sherin's to examine the alignment between groups of responses identified from the computational method and manual (human) codes of the same responses, finding that the computational approach could identify students’ conceptual understanding as veridically as human-coded responses or responses from other data sources, such as audio data. This study like Sherin’s, demonstrated that computational methods have a role in providing additional validity evidence, at least for student interview and written response data.
...
## Data Sources
Data sources for this study are the text from the middle school NGSS Performance Expectations and Oregon's AA-AAS (described above). In order to prepare these to be able to be used in the analysis, we entered the text of each of the NGSS Performance Expectations and each AA-AAS item into two spreadsheet. We then processed the data from each spreadsheet into a data structure for text data, a document-term matrix, wherein the rows represent each document and the column represents each term found across all documents. The cells contain the frequencies of each term in each document.

As part of data processing, we removed stop words (common words like "of", "a", "the", "and", "is", etc.) from the *onix*, *SMART*, and *snowball* lexicons [@lewis04; @onix; @snowball], as implemented in the *tidytext* R package [@silge16]. Additionally, we removed verbs associated with Webb's depth of knowledge levels [@webb02]. These  removals helped ensure the topics were clustered around content-related words, rather than specific verbs prevalent throughout the standards, or overly common words. The result of these steps was two document-term matrices, one for the NGSS Performance Expectations which consisted of 59 documents and 355 distinct terms and one for the AA-AAS items with XX documents and YY distinct terms.
...
Topics were estimated using the *textmodeling* package [@grun11], while the evaluation of the number of topics to extract was conducted with the *ldatuning* package [@nikita16], both of which are extensions to the  R statistical computing environment [@r]. All plots produced using the *ggplot2* package [@wickham16], and multiple plots assembled using the *patchwork* package [@pedersen17].
