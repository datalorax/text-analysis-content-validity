...
The alternate assessments reflect patterns of emphasis that are supported by state educators as indicated by the following table that highlights the balance of standard representation by grade level for science on the alternate assessment. The 8th grade Science assessment has 48 items of progressive difficulty. All items contain a preamble, prompt, three answer choices (A, B, and C, arranged horizontally), and a simple graphic design.
...
```{r modeling}
# Fit finalized model
tm_raw <- LDA(dtm, k = 16, control = list(seed = 1234))
tidy(tm_raw, "beta")
