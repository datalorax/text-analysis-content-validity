...
ggplot(aes(val)) +
  ...
  labs(x = "Mean Rating/Probability",
       y = "Density",
       title = "Distribution of topic probabilities and alignment ratings") +
...
The bottom panel of Figure \@ref(fig:densities) displays overlapping (kernel density) distributions for items when estimated by the mean SME ratings (green distribution) versus topic model probabilities (orange distribution), with separate panels by whether or not a topic-level prediction was made by the model (note, the mean values were rescaled to have a minimum value of zero and a maximum value of 1.0). As can be seen, the topic-level probability distribution closely mirrored the SME ratings when a topic-level prediction was made (e.g., Items 1, 3, and 4). When a topic-level prediction was not made (e.g., Items 2, 5, and 6), the distributions were not closely aligned and, as would be expected, the distribution centered on 0.20, although there were also a few items around 0.5 (e.g., Item 6). Importantly, as displayed by the average SME rating distribution, a number of items were judged as having adequate alignment despite no topic-level prediction being made. Items without a topic-level prediction may therefore be candidates for further review, but only these follow-up reviews would be likely to indicate lack of alignment for individual items. On average, however, items *with* topic-level predictions were `r round(exp(coef(m_binom)["predictionPrediction Made"]), 2)` times more likely to be rated as aligned to the content standards by SMEs, which was significant (`r binom_report`). 
